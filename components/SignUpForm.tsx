...
import { faUser, faEnvelope, faLock, faEye, faEyeSlash, faArrowRight, faExclamationCircle } from '@fortawesome/free-solid-svg-icons'
...
const SignUpForm = () => {
  ...
  const validate = useCallback((): boolean => {
    ...
    else if (password !== confirmPassword) newErrors.confirmPassword = t('passwordsMustMatch')
    if (!agree) newErrors.agree = t('agreeRequired')
    setErrors(newErrors)
    return Object.keys(newErrors).length === 0
  }, [fullName, email, password, confirmPassword, agree, t])
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <motion.div custom={1} variants={itemVariants} initial="hidden" animate="visible">
              <label className="text-sm text-white/70 mb-1.5 block font-medium" htmlFor="fullName">
                {t('fullNameLabel')}
              </label>
              <div className="relative">
            ...
            <motion.div custom={4} variants={itemVariants} initial="hidden" animate="visible">
              <label className="text-sm text-white/70 mb-1.5 block font-medium" htmlFor="confirmPassword">
              ...
              <div className="relative">
                ...
                >
                  <FontAwesomeIcon icon={showConfirm ? faEyeSlash : faEye} style={{ width: 14, height: 14 }} />
                </button>
              </div>
              <AnimatePresence mode="wait">
                {errors.confirmPassword && (
            ...
            <motion.div custom={5} variants={itemVariants} initial="hidden" animate="visible">
              <label className={`flex items-start gap-2.5 cursor-pointer group ${isRtl ? 'flex-row' : 'flex-row'}`}>
          ...
          <motion.p custom={7} variants={itemVariants} initial="hidden" animate="visible" className="text-center text-white/50 text-sm mt-6">
            {t('haveAccount')}{' '}
            <Link href={`/${locale}/accout/signin`} className="text-accent-light hover:underline font-medium transition-colors">
              {t('signInAlt')}
            </Link>
          </motion.p>
        </motion.div>
