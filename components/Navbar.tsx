...
export default function Navbar() {
  ...
  return (
    <nav className={`nav-glass fixed top-0 right-0 left-0 w-full z-50 ${scrolled ? 'scrolled' : ''}`} id="navbar" dir={isRtl ? 'rtl' : 'ltr'} >
        ...
        <div className="flex items-center justify-between grow lg:grow-0 lg:justify-end gap-6">
          ...
          <div className="hidden sm:flex items-center gap-3">
            {socialLinks.map(({ icon, href, label, external }) => (
              <a key={label} href={href} {...(external ? { target: '_blank', rel: 'noopener noreferrer' } : {})} className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/50 hover:text-accent-light hover:border-accent/30 hover:bg-accent/5 transition-all" aria-label={label} >
          ...
          <div className="hidden sm:flex items-center border border-white/10 rounded-xl overflow-hidden">
            <Link href={`/${locale}/accout/signin`} className="px-4 py-2 text-sm font-medium text-white/90 hover:text-white hover:bg-white/5 transition-all" >
              {t('signin')}
            </Link>
            <span className="w-px h-5 bg-white/10" />
            <Link href={`/${locale}/accout/signup`} className="px-4 py-2 text-sm font-medium hover:text-white hover:bg-white/5 transition-all text-white/90 bg-gradient-to-r from-accent to-accent-deep" >
              {t('signup')}
            </Link>
          </div>
          ...
          <button onClick={() => setMenuOpen(true)} className="lg:hidden p-2 min-w-[44px] min-h-[44px] flex items-center justify-center text-white hover:text-accent-light transition-all" aria-label="menu" >
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div key="mobile-menu" variants={menuVariants} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="fixed inset-0 z-[100] flex flex-col items-center justify-center gap-8 bg-[rgba(12,10,16,0.98)] backdrop-blur-2xl lg:hidden" dir={isRtl ? 'rtl' : 'ltr'} >
            <button onClick={() => setMenuOpen(false)} className="absolute top-6 left-6 p-3 min-w-[44px] min-h-[44px] flex items-center justify-center text-white/60 hover:text-white transition-all" aria-label="close" >
            ...
            <div className="flex items-center gap-2 sm:gap-3 pt-4 border-t border-white/10">
              <Link href={`/${locale}/accout/signin`} onClick={handleMobileNavClick} className="px-6 py-3 rounded-xl text-base font-medium text-white/50 border border-white/10 hover:text-white hover:bg-white/5 transition-all" >
                {t('signin')}
              </Link>
              <Link href={`/${locale}/accout/signup`} onClick={handleMobileNavClick} className="px-6 py-3 rounded-xl text-base font-medium text-white bg-gradient-to-r from-accent to-accent-deep" >
                {t('signup')}
              </Link>
            </div>
          </motion.div>
